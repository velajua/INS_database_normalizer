...
class DataTabsApp:
    def __init__(self, root):
        ...
        try:
            with open(os.path.join('canales_endemicos_conf',
                ...
                self.mun_col = config.get('mun', 'MUNICIPIO')
                ...
                self.mun_not_grouped = config.get('mun_not_grouped', 'nmun_proce')
    ...
    def request_column_names(self, e):
        ...
        dept_entry.pack()
        tk.Label(prompt_window, text="Columna Municipio:").pack()
        mun_entry = tk.Entry(prompt_window)
        mun_entry.pack()
        ...
        def set_columns():
            ...
            self.mun_not_grouped = mun_entry.get()
            ...
            try:
                self.data = pd.read_csv(self.file_path, sep=';',
                                        encoding=enc, usecols=[self.year_not_grouped, self.week_not_grouped,
                                                                    self.dept_not_grouped, self.mun_not_grouped, self.count_not_grouped]) if self.file_path.endswith(
                                            '.csv') else pd.read_excel(self.file_path, usecols=[self.year_not_grouped, self.week_not_grouped,
                                                                                                self.dept_not_grouped, self.mun_not_grouped, self.count_not_grouped])
                self.loaded_data = True
                break
            except Exception as e:
                self.loaded_data = False
                self.request_column_names(e)
                break
    ...
    def overwrite_base_cols_json(self):
        ...
        config['mun_not_grouped'] = self.mun_not_grouped
    ...
    def load_data(self):
        ...
        if self.file_path and not self.loaded_data and any(
            ...
            if self.chk_state.get() == 1:
                ...
                if self.loaded_data:
                    self.data = self.data.groupby([self.year_not_grouped, self.week_not_grouped,
                                                   self.dept_not_grouped, self.mun_not_grouped]
                                                ).count().reset_index()[[
                    ...
                    self.data.rename(columns={self.year_not_grouped: self.year_col, self.week_not_grouped: self.week_col,
                                            self.dept_not_grouped: self.dept_col, self.mun_not_grouped: self.mun_col,
                                            self.count_not_grouped: self.cases},
                                            inplace=True)
            ...
            if self.loaded_data:
                ...
                for col_name in self.data.columns:
                    if self.mun_col in col_name:
                        self.mun_col = col_name
                        ttk.Label(self.left_panel, text="DEPARTAMENTO -&- MUNICIPIO:").pack(padx=10, pady=5)
                        scrollbar = ttk.Scrollbar(self.left_panel, orient=tk.VERTICAL)
                        scrollbar.pack(side=tk.RIGHT, fill=tk.Y, padx=(0, 10), pady=5)
                        self.dept_data_list = [i for i in sorted(self.data[[self.dept_col, self.mun_col]].apply(
                            lambda x: f"{x[self.dept_col]} -&- {x[self.mun_col]}",
                            axis=1).unique()) if len(i.split(' -&- ')[0]) > 1]
                    ...
                    self.prompt_for_column('MUNICIPIO')
                for col_name in self.data.columns:
    ...
    def prompt_for_column(self, type_):
        ...
        def update_type_col():
            ...
            if selected_col:
                ...
                elif type_ == 'MUNICIPIO':
                    self.mun_col = selected_col
                    config['mun'] = selected_col
...
Posteriormente Seleccione 'Procesar Datos'""",
            ...
            self.load_graph(model_data, year_data_, depts[0], tab_imagen)
    ...
    def load_percentiles_mediana(self, canales_dept, year_data_, tab_datos):
        percentiles = canales_dept.iloc[:-1, 3:].apply(lambda x: x.quantile([0.25, 0.5, 0.75])).T.reset_index()
        percentiles.rename(columns={'INDEX': self.week_col}, inplace=True)
        latest_year = pd.DataFrame(canales_dept.iloc[-1, 3:].values[None, :], columns=canales_dept.columns[3:])
    ...
    def load_limits_bortman(self, canales_dept, year_data_, tab_datos):
        limites = {}
        for columna in canales_dept.iloc[:, 3:]:
        ...
        latest_year = pd.DataFrame(canales_dept.iloc[-1, 3:].values[None, :], columns=canales_dept.columns[2:])
        latest_year.rename(columns={'INDEX': self.week_col}, inplace=True)
        latest_year = latest_year.T.reset_index(drop=True)
    ...
    def load_endemic_table(self, depts, year_data_, tab_data):
        dataf_comp = self.data[(self.data[self.year_col].isin(year_data_)) & (self.data[self.dept_col].isin([i.split(' -&- ')[0] for i in depts]))
                               & (self.data[self.mun_col].isin([i.split(' -&- ')[1] for i in depts]))]
        dataf_comp = dataf_comp.rename(columns={self.week_col: 'INDEX'})
        dataf_comp = dataf_comp.pivot_table(index=[self.year_col, self.dept_col, self.mun_col], columns='INDEX', values=self.cases).reset_index()
        ...
        pt = Table(datos_frame, dataframe=dataf_comp.set_index([self.year_col, self.dept_col, self.mun_col]),
                   showtoolbar=True, showstatusbar=True,
                   width=self.right_panel.winfo_screenwidth()*2/3,
                   height=self.right_panel.winfo_screenheight()*2/3)
        pt.showIndex()
