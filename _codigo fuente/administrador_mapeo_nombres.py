...
def load_mappings():
    global mapper
    try:
        from mapeo_nombres import MAPPER
        mapper = MAPPER.copy()
    except ImportError:
        messagebox.showerror("Error", "Archivo 'mapeo_nombres.py' no encontrado o formato inválido.")
        mapper = {'DEPTO': {}, 'MUN': {}, 'BOTH': {}}
    update_listbox()
...
def update_file():
    mapper_temp = sort_dict_alphabetically(mapper)
    with open('mapeo_nombres.py', 'w') as file:
        file.write("MAPPER = {\n")
        for key, value in mapper_temp.items():
            file.write(f"    '{key}':\n    {{\n")
