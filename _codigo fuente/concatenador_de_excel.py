...
global progress_bar, root, cols_organizer, cols_dropped, \
    root_directory, xlsx_files, checkboxes, cols_kept

checkboxes = {}
root_directory = os.getcwd()
progress_bar, root = None, None
cols_organizer, cols_dropped, cols_kept = [], [], []
        ...
        for i, col in enumerate(df.columns):
            if ('unnamed' not in str(df.iloc[row, i]).lower()
                ...
                if final_col_names[i]:
                    ...
                    final_col_names[i] = str(
                        df.iloc[row, i]).strip()
    df.columns = [unidecode(col.replace("'", '').strip().strip(
        '_').upper().replace(' ', '_'))
...
def rename_and_filter_columns(df, cols_kept, closest_col,
                              sheet_name, length):
    df.columns = [
        unidecode(col.replace("'", '').strip().strip(
            '_').upper().replace(' ', '_'))
    ...
    if cols_kept:
        column_dict = list(
            set([j for i in cols_kept
                 if (j := i.split()[-1]) not in cols_dropped]))
        with open(get_filename('_logs_columnas'), 'w') as cols:
            ...
            if filename.endswith('.xlsx'):
                ...
                for sheet_name in wb.sheetnames:
                    if sheet_name != 'skip':
                        ...
                        sheet = wb[sheet_name]
                        columns = [
                            unidecode(col.value.replace("'", '').strip().strip(
                                '_').upper().replace(' ', '_'))
...
def load_vars():
    global cols_organizer, cols_dropped, cols_kept
...
def browse_directory():
    ...
    update_file_label()
    update_excel_files()
    root.geometry(f"400x{25*len(xlsx_files) + 325}")
...
if __name__ == '__main__':
    ...
    format_var = tk.StringVar(value='csv', master=root)
    extension_label = tk.Label(format_frame, text='Extensión:')
    extension_label.pack(side=tk.TOP)
    xlsx_radio = ttk.Radiobutton(format_frame, text='XLSX',
                                 value='xlsx', variable=format_var)
    xlsx_radio.pack(side=tk.TOP)
    csv_radio = ttk.Radiobutton(format_frame, text='CSV',
                                 value='csv', variable=format_var)
    csv_radio.pack(side=tk.TOP)
