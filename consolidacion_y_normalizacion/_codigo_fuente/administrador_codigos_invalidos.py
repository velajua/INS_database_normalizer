...
def load_codes():
    global codes
    try:
        with open('codigos_invalidos.json', 'r', encoding=enc) as f:
            data = json.load(f)
            codes = data.get("CODS_INV", [])
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo cargar el archivo 'codigos_invalidos.json'.\nDetalles: {str(e)}")
        codes = []
    update_listbox()
...
def update_file():
    try:
        ...
        data = {"CODS_INV": unique_codes}
        
        # Write the updated codes to the JSON file
        with open('codigos_invalidos.json', 'w', encoding=enc) as file:
