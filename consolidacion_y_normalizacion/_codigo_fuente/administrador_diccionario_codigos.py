...
def load_codes():
    global codes_dict
    try:
        with open('diccionario_codigos.json', 'r', encoding=enc) as file:
            data = json.load(file)
            codes_dict = data.get("COD_DEPT_MUN", {})
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo cargar el archivo 'diccionario_codigos.json'.\nDetalles: {str(e)}")
        codes_dict = {}
    update_listbox()
...
def update_file():
    try:
        ...
        data = {"COD_DEPT_MUN": sort_dict_alphabetically(codes_dict)}
        # Write the updated codes to the JSON file
        with open('diccionario_codigos.json', 'w', encoding=enc) as file:
