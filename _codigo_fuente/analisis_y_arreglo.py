...
ERR_COLS, CDM, CODS_INV, MAPPER = None, None, None, None

try:
    with open(os.path.join('config_analisis', 'errores_cols_salida.json'), 'r', encoding='utf-8') as file:
        data = json.load(file)
...
try:
    with open(os.path.join('config_analisis', 'diccionario_codigos.json'), 'r', encoding='utf-8') as file:
        data = json.load(file)
...
try:
    with open(os.path.join('config_analisis', 'codigos_invalidos.json'), 'r', encoding='utf-8') as file:
        data = json.load(file)
...
try:
    with open(os.path.join('config_analisis', 'mapeo_nombres.json'), 'r', encoding='utf-8') as file:
        data = json.load(file)
        ...
        if len(action.split()) == 3:
            ...
            if order == 'f':
                ...
                if cols_out not in df.columns:
                    open_popup(f'Error en columnas {cols_out} para orden "name_cod"')
                    quit()
